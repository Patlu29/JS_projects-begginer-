...
while (true) {
  ...
  if (choice === "yes") {
    ...
    if (shopNo < 0 || shopNo > 3) {
        ...
        vegList = ["Tomato", "Potato", "Cauliflower", "Cucumber", "Onion", "Meat", "Milk"]
        console.log("Today list.. Please select from below...")
        while (true) {
        console.table(vegList)
        const selectedVeg = parseInt(prompt("Which one you want..Enter(0-6) or Exit(any number): "))
        if (selectedVeg === 6) {
            console.log(`\nthanks for visiting our shop...here's your ${vegList[selectedVeg].toUpperCase()} and your bill is $10...`)
            break
        }else if (selectedVeg >= 0 && selectedVeg <= 5) {
            console.log(`\nthanks for visiting our shop...here's your ${vegList[selectedVeg].toUpperCase()} and your bill is $25...`)
            break
        }else {
